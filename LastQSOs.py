...
class LastQSOs(ttk.Frame):
    ...
    def lookup(self, callsign, band=None, mode=None):
        duplicate = False
        # Clear then repopulate with newest first
        for iid in self.tree.get_children():
            self.tree.delete(iid)
        try:
            rows = self.fetch_qsos_by_call(callsign, self.limit_var.get())
            # Display all found QSOs
            for row in rows:
                self.tree.insert("", "end", values=row)
                if band and row[self.COLUMNS.index("Band")] == band and mode and row[self.COLUMNS.index("Mode")] == mode:
